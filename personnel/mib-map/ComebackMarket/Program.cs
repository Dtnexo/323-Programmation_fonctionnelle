namespace ComebackMarket
{
    internal class Program
    {
            ...
            var producteurs = new List<Producteur>
        {
            ...
            var seller = producteurs.Select(p => new
            {
                Seller = p.Nom.Substring(0, 3) + "..." + p.Nom.Substring(p.Nom.Length - 1),
                Product = i18n[p.Produit],
                CA = p.Price * p.quantity
            }).ToList();
            Console.WriteLine("Seller" + "\tProduct" + "\tCA");
            seller.ForEach(s => Console.WriteLine(s.Seller+ "\t"+ s.Product +"\t" + s.CA));

                File.WriteAllLines("./../../../../ComebackMarket.csv", new[] { "Seller;Product;CA" }.Concat(seller.Select(s => $"{s.Seller};{s.Product};{s.CA}")));
